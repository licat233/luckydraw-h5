...
export default {
  ...
  methods: {
    ...
    setLuckyIndex() {
      const resp = axios.post(this.baseUrl + "/api/luckydraw", { passport: this.passport, activityUuid: this.activityUuid })
      resp.then(res => {
        if (!res.data.success) {
    ...
    getPrizeList() {
      const resp = axios.post(this.baseUrl + "/api/awards/list", { activityUuid: this.activityUuid })
      resp.then(res => {
        if (!res.data.success) {
          throw new Error(res.data.errorMessage)
        }
    ...
    queryAwards() {
      const resp2 = axios.post(this.baseUrl + "/api/winningRecords/query", { activityUuid: this.activityUuid, passport: this.passport })
      resp2.then(res => {
        if (!res.data.success) {
          throw new Error(res.data.errorMessage)
        }
    ...
    fetchCurrentUser() {
      const resp3 = axios.post(this.baseUrl + "/api/users/currentUser", { activityUuid: this.activityUuid, passport: this.passport })
      resp3.then(res => {
        if (res.data == null) {
          this.count = 0;
          return
